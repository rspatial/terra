...
from terra.aggregate import aggregate
...
def test_aggregate_mean_with_na():
    ...
    r = make_r()
    f = aggregate(r, 2, "mean", na_rm=False)
    result = _vals(f)
...
def test_aggregate_mean_na_rm():
    ...
    r = make_r()
    f = aggregate(r, 2, "mean", na_rm=True)
    result = _vals(f)
...
def test_aggregate_multilayer_mean():
    ...
    r = make_r()
    r2 = r * 2   # r*2 uses the operator from arith.py
    opt = pt.SpatOptions()
    rr = r.deepcopy()
    rr.addSource(r2, True, opt)

    f = aggregate(rr, 2, "mean", na_rm=False)
    result = _vals(f)
    expected = [float("nan"), 5.5, 11.5, 13.5,
...
def test_aggregate_multilayer_mean_na_rm():
    ...
    f = aggregate(rr, 2, "mean", na_rm=True)
    result = _vals(f)
...
def test_aggregate_multilayer_min_na_rm():
    ...
    f = aggregate(rr, 2, "min", na_rm=True)
    result = _vals(f)
