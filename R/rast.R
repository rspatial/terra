...
new_rast <- function(nrows=10, ncols=10, nlyrs=1, xmin=0, xmax=1, ymin=0, ymax=1, crs, extent, resolution, vals, names, time, units) {
	...
	r@pnt <- SpatRaster$new(c(nrows, ncols, nlyrs), e, crs)
	r <- messages(r, "rast")
...
setMethod("rast", signature(x="list"),
	function(x, warn=TRUE) {
		...
		opt <- spatOptions()
		for (i in 2:length(x)) {
			out@pnt$addSource(x[[i]]@pnt, warn, opt)
		}
...
setMethod("rast", signature(x="character"),
	function(x, subds=0, lyrs=NULL, drivers=NULL, opts=NULL, win=NULL, snap="near", vsi=FALSE) {
		...
		if (is.character(subds)) {
			#r@pnt <- SpatRaster$new(f, -1, subds, FALSE, 0[])
			r@pnt <- SpatRaster$new(f, -1, subds, FALSE, drivers, opts, 0[])
		} else {
			r@pnt <- SpatRaster$new(f, subds-1, "", FALSE, drivers, opts, 0[])
		}
		r <- messages(r, "rast")
		if (r@pnt$getMessage() == "ncdf extent") {
...
multi <- function(x, subds=0, xyz=3:1, drivers=NULL, opts=NULL) {
	...
	if (is.character(subds)) {
		r@pnt <- SpatRaster$new(f, -1, subds, TRUE, drivers, opts, xyz-1)
	} else {
		r@pnt <- SpatRaster$new(f, subds-1, ""[0], TRUE, drivers, opts, xyz-1)
	}
	if (r@pnt$getMessage() == "ncdf extent") {
...
setMethod("rast", signature(x="SpatRaster"),
	function(x, nlyrs=nlyr(x), names, vals, keeptime=TRUE, keepunits=FALSE, props=FALSE) {
		...
		x@pnt <- x@pnt$geometry(nlyrs, props, keeptime, keepunits)
		x <- messages(x, "rast")
		if (!missing(names)) {
