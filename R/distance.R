...
setMethod("buffer", signature(x="SpatRaster"),
	function(x, width, background=0, filename="", ...) {
		...
		x@pnt <- x@pnt$buffer(width, background, opt)
		messages(x, "buffer")
	}
)
...
setMethod("costDist", signature(x="SpatRaster"),
	function(x, target=0, scale=1, maxiter=50, filename="", ...) {
		...
		x@pnt <- x@pnt$costDistance(target[1], scale[1], maxiter, FALSE, opt)
		messages(x, "costDist")
	}
)
...
setMethod("gridDist", signature(x="SpatRaster"),
	function(x, target=0, scale=1, maxiter=50, filename="", ...) {
		opt <- spatOptions(filename, ...)
		if (is.na(target)) {
			x@pnt <- x@pnt$gridDistance(scale[1]	, opt)
		} else {
			maxiter <- max(maxiter[1], 2)
			x@pnt <- x@pnt$costDistance(target[1], scale[1], maxiter, TRUE, opt)
		}
		messages(x, "gridDist")
	}
)
...
setMethod("distance", signature(x="SpatRaster", y="SpatVector"),
	function(x, y, unit="m", rasterize=FALSE, haversine=TRUE, filename="", ...) {
		...
		if (rasterize) {
			x@pnt <- x@pnt$vectDistanceRasterize(y@pnt, NA, NA, unit, haversine, opt)
		} else {
			x@pnt <- x@pnt$vectDistanceDirect(y@pnt, unit, haversine, opt)
		}
		messages(x, "distance")
	}
)
...
setMethod("distance", signature(x="SpatVector", y="ANY"),
	function(x, y, sequential=FALSE, pairs=FALSE, symmetrical=TRUE, unit="m") {
		...
		if (sequential) {
			return( x@pnt$distance_self(sequential, unit))
		}
		unit <- as.character(unit[1])
		d <- x@pnt$distance_self(sequential, unit)
		messages(x, "distance")
		class(d) <- "dist"
		attr(d, "Size") <- nrow(x)
		attr(d, "Diag") <- FALSE
...
setMethod("distance", signature(x="SpatVector", y="SpatVector"),
	function(x, y, pairwise=FALSE, unit="m") {
		unit <- as.character(unit[1])
		d <- x@pnt$distance_other(y@pnt, pairwise, unit)
		messages(x, "distance")
		if (!pairwise) {
...
setMethod("distance", signature(x="matrix", y="matrix"),
	function(x, y, lonlat, pairwise=FALSE) {
		...
		stopifnot(ncol(y) == 2)
		v <- vect()
		d <- v@pnt$point_distance(x[,1], x[,2], y[,1], y[,2], pairwise[1], 1, lonlat)
		messages(v)
		if (pairwise) {
			d
		} else {
...
setMethod("direction", signature(x="SpatRaster"),
	function(x, from=FALSE, degrees=FALSE, filename="", ...) {
		opt <- spatOptions(filename, ...)
		x@pnt <- x@pnt$rastDirection(from[1], degrees[1], NA, NA, opt)
