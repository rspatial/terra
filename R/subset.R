...
setMethod("subset", signature(x="SpatRaster"),
	function(x, subset, negate=FALSE, NSE=FALSE, filename="", overwrite=FALSE, ...) {
		...
		x@pnt <- x@pnt$subset(subset-1, opt)
		messages(x, "subset")
	}
)
...
function(x, i, j, drop=FALSE) {
	...
	p <- x@pnt$subset_rows(i-1)
	x@pnt <- p$subset_cols(j-1)
	x <- messages(x, "'['")
	if (drop) {
		as.data.frame(x)
	} else {
