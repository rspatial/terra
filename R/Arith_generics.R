...
setMethod("Arith", signature(e1="SpatExtent", e2="SpatExtent"),
    function(e1, e2){
		oper <- as.vector(.Generic)[1]
		if (oper == "+") {
			e1@pnt <- e1@pnt$deepcopy()
			e1@pnt$union(e2@pnt)
		} else if (oper == "*") {
			e1@pnt <- e1@pnt$intersect(e2@pnt)
		} else if (oper == "/") {
...
setMethod("Arith", signature(e1="SpatVector", e2="SpatVector"),
    function(e1, e2){
		oper <- as.vector(.Generic)[1]
		if (oper == "+") {
			e1@pnt <- e1@pnt$union(e2@pnt)
		} else if (oper == "*") {
			e1@pnt <- e1@pnt$intersect(e2@pnt)
		} else if (oper == "-") {
			e1@pnt <- e1@pnt$erase(e2@pnt)
		} else {
...
setMethod("Arith", signature(e1="SpatRaster", e2="SpatRaster"),
    function(e1, e2){
		opt <- spatOptions()
		oper <- as.vector(.Generic)[1]
		e1@pnt <- e1@pnt$arith_rast(e2@pnt, oper, FALSE, opt)
		messages(e1, oper)
	}
)
...
setMethod("Arith", signature(e1="SpatRaster", e2="numeric"),
    function(e1, e2){
		oper <- as.vector(.Generic)[1]
		opt <- spatOptions()
		e1@pnt <- e1@pnt$arith_numb(e2, oper, FALSE, FALSE, opt)
		messages(e1, oper)
	}
)
...
setMethod("Arith", signature(e1="numeric", e2="SpatRaster"),
    function(e1, e2){
		oper <- as.vector(.Generic)[1]
		opt <- spatOptions()
		e2@pnt <- e2@pnt$arith_numb(e1, oper, TRUE, FALSE, opt)
		messages(e2, oper)
	}
)
...
setMethod("Arith", signature(e1="SpatRaster", e2="matrix"),
    function(e1, e2){
		oper <- as.vector(.Generic)[1]
		opt <- spatOptions()
		e1@pnt <- e1@pnt$arith_m(as.vector(e2), oper, dim(e2)[1:2], FALSE, opt)
		messages(e1, oper)
	}
)
...
setMethod("Arith", signature(e1="matrix", e2="SpatRaster"),
    function(e1, e2){
		oper <- as.vector(.Generic)[1]
		opt <- spatOptions()
		e1@pnt <- e1@pnt$arith_m(as.vector(e2), oper, dim(e2)[1:2], TRUE, opt)
		messages(e1, oper)
	}
)
...
setMethod("Compare", signature(e1="SpatRaster", e2="character"),
    function(e1, e2){
		...
		opt <- spatOptions()
		e1@pnt <- e1@pnt$arith_numb(e2, oper, TRUE, FALSE, opt)
		messages(e1, oper)
	}
)
...
setMethod("Logic", signature(e1="SpatRaster", e2="SpatRaster"),
    function(e1, e2){
		oper <- as.vector(.Generic)[1]
		opt <- spatOptions()
		e1@pnt <- e1@pnt$logic_rast(e2@pnt, oper, opt)
		messages(e1, oper)
	}
)
...
setMethod("Logic", signature(e1="SpatRaster", e2="numeric"),
    function(e1, e2){
		...
		oper <- as.vector(.Generic)[1]
		e2 <- as.logical(e2)
		e1@pnt <- e1@pnt$logic_numb(e2, oper, opt)
		messages(e1, oper)
	}
)
...
setMethod("Logic", signature(e1="numeric", e2="SpatRaster"),
    function(e1, e2){
		...
		oper <- as.vector(.Generic)[1]
		e1 <- as.logical(e1)
		e2@pnt <- e2@pnt$logic_numb(e1, oper, opt)
		messages(e2, oper)
	}
)
...
setMethod("not.na", signature(x="SpatRaster"),
	function(x, falseNA=FALSE, filename="", ...) {
		...
		x@pnt <- x@pnt$not_na(falseNA, opt)
		messages(x, "not.na")
	}
)
...
setMethod("isFALSE", signature(x="SpatRaster"),
	function(x) {
		opt <- spatOptions()
		x@pnt <- x@pnt$is_false(FALSE, opt)
		messages(x, "isFALSE")
	}
)
...
setMethod("is.bool", signature(x="SpatRaster"),
	function(x) {
		x@pnt$valueType(FALSE) == 3
	}
)
setMethod("is.int", signature(x="SpatRaster"),
	function(x) {
		x@pnt$valueType(FALSE) == 1
	}
)
...
setMethod("countNA", signature(x="SpatRaster"),
	function(x, n=0) {
		...
		n <- round(n)
		if (n == 1) {
			x@pnt <- x@pnt$anynan(FALSE, opt)
		} else {
			x@pnt <- x@pnt$countnan(n, opt)
		}
		messages(x, "countNA")
	}
)
...
setMethod("allNA", signature(x="SpatRaster"),
	function(x, falseNA=FALSE) {
		opt <- spatOptions()
		x@pnt <- x@pnt$allnan(falseNA, opt)
		messages(x, "allNA")
	}
)
...
setMethod("is.finite", signature(x="SpatRaster"),
	function(x) {
		opt <- spatOptions()
		x@pnt <- x@pnt$isfinite(FALSE, opt)
...
setMethod("is.infinite", signature(x="SpatRaster"),
	function(x) {
		opt <- spatOptions()
		x@pnt <- x@pnt$isinfinite(FALSE, opt)
...
.summarize <- function(x, ..., fun, na.rm=FALSE, filename="", overwrite=FALSE, wopt=list(), par=FALSE) {
	...
	r <- rast()
	if (is.null(add)) {
		r@pnt <- x@pnt$summary(fun, na.rm, opt)
	} else {
		r@pnt <- x@pnt$summary_numb(fun, add, na.rm, opt)
	}
	messages(r, fun)
	r
}
...
setMethod("which.max", "SpatRaster",
	function(x) {
		opt <- spatOptions()
		x@pnt <- x@pnt$summary("which.max", TRUE, opt)
...
setMethod("which.min", "SpatRaster",
	function(x) {
		opt <- spatOptions()
		x@pnt <- x@pnt$summary("which.min", TRUE, opt)
...
setMethod("which.lyr", "SpatRaster",
	function(x) {
		opt <- spatOptions()
		x@pnt <- x@pnt$summary("which", TRUE, opt)
...
setMethod("where.max", "SpatRaster",
	function(x, values=TRUE, list=FALSE) {
		opt <- spatOptions()
		out <- x@pnt$where("max", values, opt)
...
setMethod("where.min", "SpatRaster",
	function(x, values=TRUE, list=FALSE) {
		opt <- spatOptions()
		out <- x@pnt$where("min", values, opt)
...
setMethod("Compare", signature(e1="SpatExtent", e2="SpatExtent"),
    function(e1, e2){
		...
		return( e1@pnt$compare(e2@pnt, oper, 0.000001) )
	}
)
...
setMethod("modal", signature("SpatRaster"),
	function(x, ..., ties="first", na.rm=FALSE, filename="", overwrite=FALSE, wopt=list()) {
		...
		x@pnt <- x@pnt$modal(add, ties[1], na.rm[1], opt)
		messages(x, "modal")
	}
)
...
setMethod("compare", signature(x="SpatRaster"),
    function(x, y, oper, falseNA=FALSE, filename="", overwrite=FALSE, ...){
		...
		if (inherits(y, "SpatRaster")) {
			x@pnt <- x@pnt$arith_rast(y@pnt, oper, falseNA[1], opt)
		} else {
			x@pnt <- x@pnt$arith_numb(y, oper, FALSE, falseNA[1], opt)
		}
		messages(x, oper)
	}
)
...
setMethod("logic", signature(x="SpatRaster"),
    function(x, oper, falseNA=FALSE, filename="", overwrite=FALSE, ...){
		...
		if (oper == "is.infinite") {
			x@pnt <- x@pnt$isinfinite(falseNA, opt)
		} else if (oper == "is.finite") {
			x@pnt <- x@pnt$isfinite(falseNA, opt)
		} else if (oper == "is.na") {
			x@pnt <- x@pnt$isnan(falseNA, opt)
		} else if (oper == "isTRUE") {
			x@pnt <- x@pnt$is_true(falseNA, opt)
		} else if (oper == "isFALSE") {
			x@pnt <- x@pnt$is_false(falseNA, opt)
		} else if (oper == "allNA") {
			x@pnt <- x@pnt$allnan(falseNA, opt)
		} else if (oper == "noNA") {
			x@pnt <- x@pnt$nonan(falseNA, opt)
		} else if (oper == "anyNA") {
			x@pnt <- x@pnt$anynan(falseNA, opt)
		} else if (oper == "anyNA") {
			x@pnt <- x@pnt$anynan(falseNA, opt)
		} else if (oper == "!") {
			x@pnt <- x@pnt$arith_numb(0, "==", FALSE, falseNA[1], opt)
		} else {
			error("logic", "??")
		}
		messages(x, "logic")
