...
//		std::vector<double> compute_aggregates(std::vector<double> &in, size_t nr, std::vector<unsigned> dim, std::function<double(std::vector<double>&, bool)> fun, bool narm);
		...
		SpatRaster costDistanceRun(SpatRaster &old, bool &converged, double target, double m, bool lonlat, bool global, bool npole, bool spole, SpatOptions &opt);
		SpatRaster costDistance(double target, double m, size_t maxiter, SpatOptions &opt);
		...
		SpatRaster rasterizeGeom(SpatVector x, std::string unit, SpatOptions &opt);
